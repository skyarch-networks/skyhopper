...
describe Stack, type: :model do
  ...
  describe '#in_progress?' do
    context 'status is in progress' do
      let(:status_list) do
        %w[CREATE_IN_PROGRESS ROLLBACK_IN_PROGRESS DELETE_IN_PROGRESS UPDATE_IN_PROGRESS UPDATE_COMPLETE_CLEANUP_IN_PROGRESS UPDATE_ROLLBACK_IN_PROGRESS UPDATE_ROLLBACK_COMPLETE_CLEANUP_IN_PROGRESS]
      end
    ...
    context 'status is not in progress' do
      let(:status_list) do
        %w[CREATE_FAILED CREATE_COMPLETE ROLLBACK_FAILED ROLLBACK_COMPLETE DELETE_FAILED DELETE_COMPLETE UPDATE_COMPLETE UPDATE_ROLLBACK_FAILED UPDATE_ROLLBACK_COMPLETE]
      end
