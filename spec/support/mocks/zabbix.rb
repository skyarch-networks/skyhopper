...
module ZabbixStub
  def stubize_zabbix
    ...
    before do
      ...
      allow(_zabbix).to receive(:get_hostgroup_ids).and_return(%w[1 2 3]) # .with(String or Array of String)
      ...
      allow(_zabbix).to receive(:get_history).with(str, str).and_return([%w[foo bar]])
