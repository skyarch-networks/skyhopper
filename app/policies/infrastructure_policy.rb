...
class InfrastructurePolicy < ApplicationPolicy
  ...
  %i[edit? update? destroy? delete_stack? change_rds_scale? save_schedule? icalendar? upload_calendar? rds_submit_groups? start_rds? stop_rds? reboot_rds?].each do |action|
    define_method(action) do
