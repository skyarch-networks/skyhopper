...
class Infrastructure < ActiveRecord::Base
  ...
  def resources_updated?
    old_physical_ids = resources.pluck(:physical_id)
    stack = Stack.new(self)
    now_physical_ids = stack.instances_for_resources.map(&:physical_resource_id)
    old_physical_ids.sort != now_physical_ids.sort
  end
  ...
  def detach_zabbix
    zabbix_server_id = project.zabbix_server_id
