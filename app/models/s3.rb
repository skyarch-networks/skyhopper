...
class S3 < SimpleDelegator
  ...
  def index_document
    web_conf_opt[:index_document][:suffix] if website?
  end

  def error_document
    web_conf_opt[:error_document][:key] if website? and web_conf_opt[:error_document]
  end

  def public_url
    "http://#{name}.s3-website-#{@region}.amazonaws.com" if website?
  end
end
